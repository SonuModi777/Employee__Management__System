...
public class TakeAttendance extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        try{
            conn c1 = new conn();
            String q = "select id from attendence where id='"+id+"'";
            ResultSet rs = c1.s.executeQuery(q);
            if(rs.next()){
                JOptionPane.showMessageDialog(null, "Attendance updated for this id");
            }else{
                c1.s.executeUpdate(qry);
                JOptionPane.showMessageDialog(null, "Attendance Confirmd");
